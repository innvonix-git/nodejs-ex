...
exports.showUserProfile = async function(req, res, next){
	...
	let followers = [];
	let follower = {}
	follower.fullname = "ABC";
	follower.profile_pic = "my_pic.jpg";
	follower.user_id = "1";
	follower.is_following = 0;
	followers.push(follower);
	followers.push(follower);
	follower.is_following = 1;
	followers.push(follower);
	...
	allFollowers.forEach(function(data){
		console.log(data);
		Follower.aggregate([{$match : {follower_user: ObjectId(data.follower_user)}},{ $lookup: { from: "users", localField: "follower_user", foreignField:"_id", 
		as: "user"}},{ $unwind: "$user"} ]).collation({ locale: "en" })
	...
	allFollowings.forEach(function(data){
		Follower.aggregate([{$match :{$or: [{$and:[{following_user: ObjectId(data.following_user)}, 
			{follower_user:ObjectId(profile._id)}]}]}},{ $lookup: { from: "users", localField: "following_user", foreignField:"_id", 
		as: "user"}},{ $unwind: "$user"} ]).collation({ locale: "en" })
		.sort({'user.first_name':ordered_sort}).exec(function (err, followingsdata){
			if(err){
				...
				followingsName.push({'fullname':followingsdata[0].user.first_name+" "+followingsdata[0].user.last_name, '_id': followingsdata[0].user._id});
			}			
		});	
	});
...
exports.showFollowers = async function(req, res){
	...
	const ObjectId = mongoose.Types.ObjectId;
	 ...
	 let user_id = req.query.userId;
	allFollowers = await Follower.aggregate([{ $lookup: { from: "users", localField: "following_user", foreignField:"_id", 
		as: "user"}},{ $unwind: "$user"}, {$match : {following_user: ObjectId(user_id)}} ]).collation({ locale: "en" })
...
exports.showFollowings = async function(req, res){
	...
	const ObjectId = mongoose.Types.ObjectId;
	 ...
	 let user_id = req.query.userId;
	allFollowings = await Follower.aggregate([{ $lookup: { from: "users", localField: "follower_user", foreignField:"_id", 
		as: "user"}},{ $unwind: "$user"}, {$match : {follower_user: ObjectId(user_id)}} ]).collation({ locale: "en" })
		.sort({'user.first_name':ordered_sort}).skip(skip).limit(perpage).exec();
 		...
 		if(err){
			...
			if (userData){
				...
				userData.save(function(err) {
            		...
            		{
            			res.send({'status' : 'true', 'message' : 'auto renew updated successfully.', 'data': null});
                	}
                });
			}
		}
					...
					Membership.findOne({_id:mid},function(err,currentmember){
						...
						if(currentmember){
							...
							if(token_balance >= requested_membership_cost){
								...
								req.flash('error', 'You have not enough token to upgrade membership');
								res.redirect('/user-memberships');
							}
...
exports.showSearchPage = async function(req, res) {
	...
	/*if (submited_data.location.length == 0) {
	...
	properties = await Property.find({ $and: [{ $or: [{ property_name: { "$regex": keyword, "$options": "i" } }, { address1: { "$regex": keyword, "$options": "i" } }, { address2: { "$regex": keyword, "$options": "i" } }, { post_code: { "$regex": keyword, "$options": "i" } }, { category_id: { $in: categoriesToSearch } }, { area: { $in: locationsToSearch } }] }, { status: 1 }] }).limit(perPage).skip(perPage * page).sort({ property_name: 'asc'}).exec();
	*/
	sproperties = await Property.find({ status: 1, is_featured:1 }).limit(3);	
	properties.forEach(function(property) {
	...
	console.log("submited_data.location");
	console.log(submited_data.location);
	//Get Categories Data
	...
	console.log("search count:" + counter+"current page"+page);
