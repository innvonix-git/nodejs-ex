...
exports.UpdateProfile = async function (req, res) {
	...
	var fileName = '';
	var db_records = parseInt(req.body.db_records);

	if (req.file !== undefined) {
		fileName = req.file.filename;
		generateThumb('public/uploads/' + fileName, '345px', 'thumb_', 'public/uploads/thumbs/');
		var db_images = user.profile_photo;

		if(fs.existsSync('public/uploads/'+db_images)){
			fs.unlink('public/uploads/'+db_images, (err) => {
				if (err){
		...
		if(fs.existsSync('public/uploads/thumbs/thumb_'+db_images)){
			fs.unlink('public/uploads/thumbs/thumb_'+db_images, (err) => {
			if (err){
			  	console.log('file delete error');
			  }else{
			  	console.log('File deleted!');
		...
		if(db_records==0){
			...
			fileName = user.profile_photo;
				...
				user.business_address1 = req.body.business_address1;
				user.business_area = req.body.business_area;
				user.city = req.body.city_name;
				user.country = req.body.country_name;
				user.postcode = req.body.post_code;
